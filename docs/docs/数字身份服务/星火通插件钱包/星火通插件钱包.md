...
### 应用场景
星火通插件钱包是一种全新的数字身份管理方式。 您可以通过桌面端浏览器出示自己的可信身份证明，来完成星火链网各类公共服务系统登录时的身份验证， 更快更轻松的获得星火区块链服务。
### 主要功能
**BID账号管理**</br>为用户提供身份的创建、更新及验证功能。用户可以使用插件钱包自主生成BID，作为使用星火链网的账号；也可以将已有BID对应的助记词、私钥等导入插件钱包，恢复已有的BID账号。</br>
...
##### 出示并验证数字凭证
调用该方法可以选择性出示数字凭证声明，且星火链网验证服务会对该声明进行验证，验证通过后会返回"验证通过证明"、"证明内容"及"验证方公钥"。
```javascript
var obj = {
