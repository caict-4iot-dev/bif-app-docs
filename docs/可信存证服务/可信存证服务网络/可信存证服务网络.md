...
访问方式二：直接访问可信存证服务网络地址，生产环境地址：https://stamp.bitfactory.cn/ ，测试环境地址：https://test-stamp.bitfactory.cn/ 。   
...
| ---- | ---- | ------------------------------- |
| file | 是   | 文件                              |
| type | 是   | doc：文档；pic：图片；audio：音频；video：视频 |
...
| fileLabel         | 文件标签         | 是   |
| list              | HashInfo对象列表 | 是   |
| HashInfo.filename | 文件名          | 是   |
| HashInfo.fileHash | 文件hash       | 是   |
...
| 参数名             | 描述                       | 是否必填 |
| --------------- | ------------------------ | ---- |
| evidenceType    | 存证类型 1.文件存证 2.hash存证     | 否  |
| evidenceChannel | 存证方式 1.自助 2.API          | 否  |
| state           | 3.待支付4.上链中5.存证成功6.存证失败   | 否  |
| startTime       | 开始时间 2023-08-18 00:00:00 | 否  |
| endTime         | 结束时间 2023-08-18 00:00:00 | 否  |
| pageNumber      | 当前页码                     | 否  |
| pageSize        | 每页显示数量 最大50              | 否  |
| filename        | 文件名称                     | 否  |
