...
星火服务平台 面向星火·链网用户，包含政府、企业、骨干节点，提供了星火·链网统一的 用户中心、控制台总览。支持骨干节点申请、开放平台API申请等基础服务，同时集成了其他产品服务的使用入口，辅助其他产品的使用，便于用户统一进行管理。

> 访问地址：https://bj-baas.bitfactory.cn/ （北京超级节点服务平台为例）

### 应用场景

**场景一：骨干节点接入星火·链网**

企业想要建设骨干节点，需先通过星火服务平台-基础业务平台，进行骨干节点申请、开放平台API调用申请等，通过后，才能开展后续业务。

**场景二：使用超级节点产品服务**

星火服务平台集成了超级节点各个产品服务入口，星火·链网用户（企业、政府、骨干节点）可通过星火服务平台产品菜单页面，切换到各个产品服务平台中，便于用户操作。
...
**账户去中心化：**星火服务平台去掉传统的 统一账号注册和登录的方式，改用插件钱包来替换系统身份。平台不再托管用户私钥，由用户自主管理私钥，保护用户隐私和权益。

**产品服务统一入口：**服务平台集成超级节点所有产品服务入口，便于用户进行统一管理。
...
星火·链网服务平台通过星火·浏览器插件钱包链接，星火用户需要通过插件钱包登录到星火服务平台。如未案装插件钱包，请参照插件钱包产品手册进行下载、安装、创建数字身份。

![img](image/wps2.jpg) 

操作步骤：

1. 点击「快捷授权登录」唤起星火·浏览器插件钱包，如未安装插件，需按引导案装界面进行案装。

2. 唤起插件钱包，选择授权登录的账户 。

3. 出示可信认证（如未进行可信认证，需根据插件钱包引导进行企业/个人可信认证申请）。

4. 若授权账户地址是该分级管理员，需出示星火个人可信认证。
...
登录后可点击进入我的账户：展示用户的账户信息、星火可信认证信息以及身份授权管理。

（1） 用户未出示可信认证前：我的账户页面只展示基本信息。

（2） 账户已出示星火可信认证时，展示全部板块内容：

![img](image/wps4.jpg) 

  a) 基本信息：展示主体BID、所属主体、入驻时间等。

  b) 身份授权管理：展示出该主体下的主体身份和节点身份，对其可以进行成员授权及管理。

  c) 点击管理员：支持新增分级管理员，删除以及权限分配。

### 3 骨干节点接入

#### 3.1 申请骨干节点

用户完成数字身份创建、企业可信认证申请后，可在星火服务平台进行骨干节点申请。申请前还需要准备：

1. 使用BID-SDK-JAVA线下生成一个数据网关账户地址、跨链网关账户地址、骨干节点账户地址（公钥私钥也要保存好），为下一步申请成为骨干节点时配置接入网关地址时使用（可以生成多个）。

2. 以上账户地址的生成，也可直接通过插件钱包创建。

 

**步骤：**

1. 通过插件钱包，授权企业身份，登录星火服务平台。

![img](image/wps5.jpg) 

2. 点击左上侧菜单按钮，进入产品服务菜单页，点击基础业务平台。

![img](image/wps6.jpg) 

3. 点击节点管理-节点申请-申请骨干节点。

![img](image/wps7.jpg) 

说明：

（1）申请骨干节点入口，且一个主体支持申请多个骨干节点，目前没有上限。
...
（1) 节点名称：输入节点的名称，节点名称长度不能超过 25。

（2) 节点 BID：输入骨干节点 BID，该 BID 需要是主链的地址（即不带AC号），且未被其它骨干节点占用，且需要是一个已激活的账户地址（可通过转账激活），不能和企业进行绑定（即该bid不能申请过企业可信认证）。

（3) 节点地理位置：通过选择省/直辖市，联动管理城市及县区。

（4) 节点介绍：对节点介绍，长度不能超过 200。

（5) 联系人：填写联系人姓名。

（6) 联系人电话：填写联系人电话，用于审核时联系节点申请人。

（7) 联系人邮箱：填写联系人邮箱，用于审核时联系节点申请人用点击下一步进入。
...
（1) 点击「返回上一步」回到填写节点基本信息。

（2) 点击「下一步」进入填写链基本信息。

（3) 选择文件仅支持 PDF 版本，且每个文件不能超过 10M。
...
（1) AC号：可自定义，最终以超级节点审核结果为准（超级节点审核时可修改AC号）。

（2) 解析服务网络地址：子链解析服务网络地址（子链开发bid注册解析服务时用到）。

（3 ) 链架构类型：如链架构类型中，没有子链的链结构类型，可联系运营人员进行添加。
...
1. 进入基础业务平台-开放平台API管理-APIkey申请，选择【骨干节点接入 立即开通】

![img](image/wps11.jpg) 

2. 进入骨干节点接入所涉及的API接口介绍页面，点击“立即开通”，开通时，需选择骨干节点账户地址签名，即前一步申请骨干节点时填写的骨干节点账户地址

​      说明：如该骨干节点账户地址未导入到插件钱包中，需先导入。

![img](image/wps12.jpg) 

3. 开通完成后 可以在开放平台API管理-我的APIKey 中查询开发者信息
...
点击平台左上角，展示星火服务平台上架的子平台或服务，具体如下：

![img](image/wps14.jpg) 

数字身份服务平台：详见数字身份平台产品手册。

链上签：详见链上签产品手册。

智能合约IDE：详见链上智能合约IDE产品手册。
...
用于骨干链、异构链数据上报到主链（要求有变动时将数据同步上来）。![image-20231019180408219](C:\Users\李瑾\AppData\Roaming\Typora\typora-user-images\image-20231019180408219.png)
...
服务商认证需要用户支付1星火链令。服务商认证申请需要两步操作：

（1）获取服务商认证blob，用于将服务商认证费用上链；

（2）用户私钥对blob进行签名，并提交上链。
...
网关节点地址需要在主链上激活，请联系信通院运营人员。

(2) 调用接口时，报错“余额不足”。

部分接口调用需要消耗星火令，提示余额不足时，请联系信通院运营人员。

(3) 链运行状态是为什么是“部署中”或者“运行异常”？
